...
class SeriesSerializer(serializers.ModelSerializer):
    ...
    next_cover_is_fallback = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = (
            ...
            "next_volume", "cart_count", "next_cover_url", "next_cover_is_fallback",
            "first_volume_cover_url",
            "availability_status", "availability_map", "created_at", "updated_at",
        )
        read_only_fields = ("id", "current_volume", "created_at", "updated_at")

    def _next_volume_cover(self, obj):
        """next_volume のキャッシュ済み VolumeCover（無ければ None）。"""
        return next(
    ...
    def get_next_cover_url(self, obj):
        ...
        """
        cover = self._next_volume_cover(obj)
        if cover:
    ...
    def get_next_cover_is_fallback(self, obj):
        """next_cover_url が 1 巻フォールバックか（= next_volume が未キャッシュ）。

        フロントはこれが True のとき、ホーム再表示時に裏で実表紙を取りに行く。
        next_volume の表紙がキャッシュ済みになれば False となり、再取得は止まる。
        """
        return self._next_volume_cover(obj) is None
